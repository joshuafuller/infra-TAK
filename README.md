...
- **Field evidence on tak-10, Apr 30 2026 (the OTHER half of the v0.8.7 story):** After confirming the env var fix is needed, we also discovered runtime state drift — even with proper config, server CPU can pin at p50 140%+ after days of uptime. The cure is `docker compose up -d --force-recreate --no-deps server worker` (server p50 140% → 3.3% in 3 minutes, durably, zero LDAP impact).
