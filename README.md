...
**Latest release: v0.9.13-alpha** — **AUTHENTIK ADMIN RECOVERY.** Small additive release. New **Protected Admin Accounts** panel on `/authentik` shows `is_active` status for `akadmin` and `webadmin`; if an operator accidentally clicks **Disable** in TAK Portal (or **Deactivate** in Authentik's own UI) the panel surfaces a red banner and a one-click **Reactivate** button. Recovery is layered — Authentik API with the bootstrap token first, falling back to `docker exec authentik-server-1 ak shell` (Django ORM) so it works even when the bootstrap token has been revoked or an auth flow is broken. Also: one-line fix to the existing **Sync webadmin to Authentik** button so it now re-activates a disabled webadmin (it was previously only fixing `is_superuser`/`path`/`groups`). No migrations, no operator pre-flight — Update Now is drop-in if you're already on v0.9.12. See **[docs/RELEASE-v0.9.13-alpha.md](docs/RELEASE-v0.9.13-alpha.md)** for the full notes. Prior releases: [v0.9.12](docs/RELEASE-v0.9.12-alpha.md) (cyber security hardening — port-exposure lockdown + post-auth route fixes + ReputationPolicy `negate=True` fix; **important if you haven't updated past v0.9.11 yet**), [v0.9.11](docs/RELEASE-v0.9.11-alpha.md) (security hotfix — CloudTAK PG_MEM/PGMiner; **read this if you haven't updated past v0.9.10 yet**), [v0.9.10](docs/RELEASE-v0.9.10-alpha.md), [v0.9.9](docs/RELEASE-v0.9.9-alpha.md), [v0.9.8](docs/RELEASE-v0.9.8-alpha.md), [v0.9.7](docs/RELEASE-v0.9.7-alpha.md), [v0.9.6](docs/RELEASE-v0.9.6-alpha.md), [v0.9.5](docs/RELEASE-v0.9.5-alpha.md), [v0.9.4](docs/RELEASE-v0.9.4-alpha.md), [v0.9.3](docs/RELEASE-v0.9.3-alpha.md), [v0.9.2](docs/RELEASE-v0.9.2-alpha.md), [v0.9.1](docs/RELEASE-v0.9.1-alpha.md), [v0.9.0](docs/RELEASE-v0.9.0-alpha.md) — older releases on the [GitHub Releases tab](https://github.com/takwerx/infra-TAK/releases).
