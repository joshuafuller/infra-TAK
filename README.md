...
**Headline: proactive LDAP routing migration + verifier hardening (responder field test)**
...
- **Dual-signal spiral detection** — field testing of v0.8.4 on busy boxes (Mission API / DataSync) showed high-volume `Bind request` traffic could push spiral markers off the `--tail 200` window. New `_detect_authentik_ldap_spiral()` confirms a spiral via **either** signal: ≥2 unique markers in last **1000** outpost log lines (was 200), **or** ≥30 connections in `idle in transaction` from `application_name LIKE '%authentik%'` in Postgres. The Postgres signal is durable — survives container recreates and can't be drowned out by bind volume.
...
- **Granular gate logging** — every early-return in the routing repair now logs why it skipped, under the `routing repair: ...` / `proactive routing: ...` prefix. Silent skips can no longer hide bugs.
