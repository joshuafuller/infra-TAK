...
**Latest release: v0.9.2-alpha** — **TAK Server Snapshots + Rollback + Console Rollback + Authentik Reputation Policy + SSH Fail2ban Jail + Node-RED hardening + three stabilisation bug fixes.** **(1) Authentik Reputation Policy** — flow-level brute-force blocking inside Authentik itself (every failed login decrements the source-IP score; block when score ≤ threshold). Applied automatically on first startup. UI card on the Authentik page. **(2) SSH Fail2ban Jail** — opt-in Fail2ban jail for SSH brute-force on the host, with Guard Dog email alerts, configurable thresholds, and whitelist. **(3) TAK Server Snapshots** — pre-upgrade snapshot taken automatically before every TAK Server upgrade (upgrade aborts if snapshot fails). Manual snapshots and scheduled snapshots (daily/weekly systemd timer). Download any snapshot as `.tar.gz` for off-box backup. **(4) TAK Server Rollback** — restore any snapshot with one click: stops TAK Server, restores CoreConfig, certs, and PostgreSQL database, then restarts. Rollback now also restarts Node-RED so mission state resyncs from the clean database (previously stale Node-RED mission polls caused TAK Server NPEs after restore). **(5) Console Rollback** — "Update Now" now records the previous version. A yellow rollback bar appears on the Console page after updates; one click fetches the previous GitHub tag and restarts the console. **(6) Node-RED hardening** — image pinned to `nodered/node-red:4.0`, `cap_drop: ALL` + `no-new-privileges:true`, `user: 1000:1000`, `mem_limit: 2g`, scoped per-file cert mounts, remote-deploy port binding fixed to `127.0.0.1:1880`. Applied automatically on next "Update Now". **Bug fixes:** TAK Portal update no longer stalls on git conflicts (patches now live in `docker-compose.override.yml`, not the tracked file); `git safe.directory` is now set system-wide so TAK Portal and CloudTAK updates always succeed regardless of which user owns the repo; TAK Server rollback restarts Node-RED to eliminate post-restore mission-API crashes. See **[docs/RELEASE-v0.9.2-alpha.md](docs/RELEASE-v0.9.2-alpha.md)**. Prior: **v0.9.1-alpha** — **Fail2ban reliability fixes.** Three gaps in v0.9.0 patched: (1) install now verifies the daemon actually started and fails clearly if it didn't — no more silent success with a dead service; (2) toggling any jail ON now auto-starts the daemon if it's stopped, and a warning banner appears on the Fail2ban page when the daemon is inactive; (3) the Console module card now shows the "Fail2ban" name next to the logo. See **[docs/RELEASE-v0.9.1-alpha.md](docs/RELEASE-v0.9.1-alpha.md)**. Prior: **v0.9.0-alpha** — **Custom Identification Banners + Fail2ban Brute-Force Protection.** Two operator-facing features. **Custom Banner:** a persistent fixed banner at the top of every page showing agency text, logo, and server IP/FQDN — eliminates the "which box am I on?" problem when managing a fleet. Configured at `/customization` (new sidebar page). Font size, text color, and agency logo all configurable. **Fail2ban:** host-level brute-force IP blocking, shipped as an opt-in Marketplace module. Two jails: **Authentik** (blocks repeated login failures) and **TAK Server** (blocks cert-probe bots). Both have enable/disable toggles, configurable thresholds, IP whitelists, clickable "watching" panels, animated refresh, and Guard Dog email alerts on every auto-ban. Emails include the server nickname. Log forwarder self-heals if the service file is missing on older installs. See **[docs/RELEASE-v0.9.0-alpha.md](docs/RELEASE-v0.9.0-alpha.md)**. Prior: **v0.8.9-alpha** — **Authentik security: real client IP fix (fleet-wide silent bug).** Every infra-TAK install since the Caddy→Authentik wiring shipped has logged `client_ip: "172.18.0.1"` (the Docker bridge gateway) for every login event — audit logs, failed-login events, everything. Root cause: `AUTHENTIK_LISTEN__TRUSTED_PROXY_CIDRS` defaults to "trust nothing" in Authentik. Caddy forwards `X-Forwarded-For` correctly but Authentik discards it. Same silent-default pattern as the v0.8.7 worker-count bug (same official docs page). Fix: new idempotent migration `_authentik_fix_trusted_proxy_cidrs` appends `AUTHENTIK_LISTEN__TRUSTED_PROXY_CIDRS=172.16.0.0/12,127.0.0.1/32,::1/128` to `~/authentik/.env`, triggers a server+worker recreate (LDAP outpost untouched). Wired into both `_startup_migrations` and `_post_update_auto_deploy`. Verifier extended to probe `listen.trusted_proxy_cidrs` via `ak dump_config`. Persists `last_outcome` to `settings.authentik_trusted_proxy_cidrs_fix`. Migration window: ~35-60s on first upgrade; sub-second no-op on every restart after that. See **[docs/RELEASE-v0.8.9-alpha.md](docs/RELEASE-v0.8.9-alpha.md)**. Prior: **v0.8.8-alpha** — **Authentik stability: two fleet-wide latent bugs fixed.** Both have been present for several releases and both auto-heal on the next console restart after upgrade. **Bug #1 — LDAP flow stage-binding recursion.** Every infra-TAK install since the LDAP feature shipped has had `evaluate_on_plan=true` AND `re_evaluate_policies=true` on all three `ldap-authentication-flow` stage bindings — a combination that causes cascading policy re-evaluation on every step of every authentication plan. Fast-disk boxes hide it; slow-disk boxes pin Postgres at hundreds of percent CPU. Fixed: `evaluate_on_plan: false` on those three bindings (matches `default-authentication-flow`, which has zero recursion). **Bug #2 — `idle_in_transaction_session_timeout=30s` was too aggressive.** Set in v0.8.4 to bound zombie connections; on slow disks Authentik's Django startup migrations exceed 30s in idle-in-tx state and get killed mid-flight, leaving stale advisory locks that crash-loop the server forever. Bumped to `300s` (10x headroom, still bounds zombies to 5 min). Three pieces shipped: (1) **Blueprint YAML + healing function** — `evaluate_on_plan` flipped to `false` everywhere it's emitted (initial deploy + post-update reimport + `_ensure_ldap_flow_authentication_none()` healing path). (2) **NEW idempotent migration `_authentik_fix_pg_idle_timeout(plog)`** — detects 30s in `docker-compose.yml`, rewrites to 300s, force-recreates Postgres (which also clears any stale advisory locks left by previous crash loops), restarts server + worker. Runs **before** the recursion fix so the recursion-fix server restart has 10x the headroom on slow boxes. (3) **NEW idempotent migration `_authentik_fix_ldap_flow_recursion(plog)`** — counts bad bindings via SQL; if `count > 0`, runs UPDATE and restarts `authentik-server-1` ONLY (LDAP outpost untouched — cardinal rule). Both migrations wired into `_startup_migrations` AND `_post_update_auto_deploy`, persist outcome to `settings.json` (`authentik_pg_idle_timeout_fix` and `authentik_ldap_flow_recursion_fix`) for operator audit. Verifier extended to probe `idle_in_transaction_session_timeout` and re-runs after the timeout fix so audit reflects post-migration state. **No UI changes.** Rollback feature pushed to **v0.9.0+** — the v0.8.x line is reserved for Authentik stabilization. See **[docs/RELEASE-v0.8.8-alpha.md](docs/RELEASE-v0.8.8-alpha.md)**. Prior: **v0.8.7-alpha** (Authentik env var name fix — `AUTHENTIK_WEB__WORKERS` double-underscore — plus official tunings and a runtime-config verifier). **v0.8.6-alpha** (Azure / NAT deploy reliability: four field bugs). **v0.8.5-alpha** (proactive LDAP routing migration + gunicorn timeout bump). **v0.8.4-alpha** (reverses v0.8.0 LDAP outpost routing for spiraling boxes). **v0.8.3-alpha** (`idle_in_transaction_session_timeout` 120s→30s — superseded by v0.8.8's 300s). **v0.8.2-alpha** (`AUTHENTIK_WEB_WORKERS=4` — superseded by v0.8.7's correct double-underscore name). **v0.8.1-alpha** (LDAP migration conditional hotfix). **v0.8.0-alpha** (LDAP outpost TLS fix for fresh installs). **v0.7.9-alpha** (Authentik reconfigure waits for API ready). Older: [v0.7.0-alpha](docs/RELEASE-v0.7.0-alpha.md), [v0.6.8-alpha](docs/RELEASE-v0.6.8-alpha.md), [v0.6.7-alpha](docs/RELEASE-v0.6.7-alpha.md), [v0.6.5-alpha](docs/RELEASE-v0.6.5-alpha.md), [v0.6.4-alpha](docs/RELEASE-v0.6.4-alpha.md), [v0.6.1-alpha](docs/RELEASE-v0.6.1-alpha.md), [v0.6.0-alpha](docs/RELEASE-v0.6.0-alpha.md), [v0.5.9-alpha](docs/RELEASE-v0.5.9-alpha.md), [v0.5.8-alpha](docs/RELEASE-v0.5.8-alpha.md), [v0.5.7-alpha](docs/RELEASE-v0.5.7-alpha.md), [v0.5.6-alpha](docs/RELEASE-v0.5.6-alpha.md), [v0.5.5-alpha](docs/RELEASE-v0.5.5-alpha.md), [v0.5.4-alpha](docs/RELEASE-v0.5.4-alpha.md), [v0.5.3-alpha](docs/RELEASE-v0.5.3-alpha.md), [v0.5.2-alpha](docs/RELEASE-v0.5.2-alpha.md), [v0.5.1-alpha](docs/RELEASE-v0.5.1-alpha.md), [v0.5.0-alpha](docs/RELEASE-v0.5.0-alpha.md), [v0.4.9-alpha](docs/RELEASE-v0.4.9-alpha.md), [v0.4.8-alpha](docs/RELEASE-v0.4.8-alpha.md), [v0.4.7-alpha](docs/RELEASE-v0.4.7-alpha.md), [v0.4.6-alpha](docs/RELEASE-v0.4.6-alpha.md), [v0.4.5-alpha](docs/RELEASE-v0.4.5-alpha.md), [v0.4.4-alpha](docs/RELEASE-v0.4.4-alpha.md), [v0.4.3-alpha](docs/RELEASE-v0.4.3-alpha.md), [v0.4.2-alpha](docs/RELEASE-v0.4.2-alpha.md).
