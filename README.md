...
**Latest release: v0.8.5-alpha** — **Proactive LDAP routing migration + gunicorn timeout bump + verifier hardening.** Four field issues fixed: (1) **The reactive v0.8.4 detector cannot catch latent misroutes.** A TAK-installed box (`responder`) was on internal direct routing with the cached `adm_ldapservice` session masking every failure — there was no spiral signature in the outpost log because the only client successfully binding was the cached SA. The bug was invisible until `webadmin` (no cache) attempted a fresh bind, which immediately recursed. v0.8.5 adds proactive `_ensure_authentik_ldap_outpost_on_fqdn` that migrates outpost to FQDN routing **before** a spiral, gated on `/opt/tak` installed + FQDN configured + Caddy reachable. Runs on Authentik deploy, TAK Server deploy, every Update Now, and every 10 min via the spiral monitor. (2) **The verifier destroyed user records on inconclusive probes.** `_test_ldap_bind_dn` returned `False` when `ldapsearch` was missing AND outpost showed `exceeded stage recursion depth` (spiral signature, not credential failure), which triggered DELETE+POST recreate of `webadmin` and led to `400 username must be unique`. v0.8.5 adds tri-state `_test_ldap_bind_dn_verdict` (`'ok' | 'fail' | 'inconclusive'`); destructive recovery only on confirmed-fail, with re-query before POST to prevent the duplicate. (3) **High bind volume hides spiral markers AND general markers cause false positives.** v0.8.4 sampled `--tail 200`; busy boxes pushed markers off the window. v0.8.5 dev testing on tak-10 caught the inverse: treating `failed to execute flow` and `EOF` as spiral markers tripped on transient restart artifacts even on healthy boxes. v0.8.5 ships **two-tier dual-signal detection**: ≥1 spiral-specific marker (`result code 50`, `nil pointer`, `exceeded stage recursion`, 502, 503) OR Postgres `idle in transaction` ≥30 in `--tail 1000`. General markers tracked for forensics only. (4) **Gunicorn worker timeout 30s → 120s.** Even with FQDN routing correct, tak-10 (3.5+ binds/sec sustained) saw periodic Caddy 502s and brief outpost recursion. Authentik server logs showed `WORKER TIMEOUT → SIGABRT → Booting worker` — Authentik 2026.2.2's flow planner exceeds gunicorn's upstream-default 30s timeout under heavy load, gunicorn kills the worker mid-request, in-flight TCP connections drop, Caddy returns 502, outpost retries, recursion. v0.8.5 adds `_ensure_authentik_gunicorn_timeout` that appends `GUNICORN_CMD_ARGS=--timeout=120` to `~/authentik/.env` once and recreates only the server container. Idempotent (never overwrites operator override), safe everywhere (timeout never fires on fast boxes), one-shot (NOT in the periodic monitor). Periodic 10-min monitor (PID-locked, max 1 repair per 6h). Granular gate logging at every early-return. No UI changes, pure backend self-healing. See **[docs/RELEASE-v0.8.5-alpha.md](docs/RELEASE-v0.8.5-alpha.md)**. Prior: **v0.8.4-alpha** — reverses v0.8.0 LDAP outpost routing for spiraling boxes. Prior: **v0.8.3-alpha** — `idle_in_transaction_session_timeout` 120s→30s. Prior: **v0.8.2-alpha** — auto-set `AUTHENTIK_WEB_WORKERS=4`. Prior: **v0.8.1-alpha** — LDAP migration conditional hotfix. Prior: **v0.8.0-alpha** — LDAP outpost TLS fix for fresh installs. Prior: **v0.7.9-alpha** — Authentik reconfigure waits for API ready. Prior: [v0.7.0-alpha](docs/RELEASE-v0.7.0-alpha.md) (IPAWS KML network link, zone polygons, NAPSG icons), [v0.6.8-alpha](docs/RELEASE-v0.6.8-alpha.md) (Node-RED deploy sync fix), [v0.6.7-alpha](docs/RELEASE-v0.6.7-alpha.md) (DataSync read-only missions, shared missions), [v0.6.5-alpha](docs/RELEASE-v0.6.5-alpha.md) (KML + ArcGIS stable-ID / Purge), [v0.6.4-alpha](docs/RELEASE-v0.6.4-alpha.md). Older: [v0.6.1-alpha](docs/RELEASE-v0.6.1-alpha.md), [v0.6.0-alpha](docs/RELEASE-v0.6.0-alpha.md), [v0.5.9-alpha](docs/RELEASE-v0.5.9-alpha.md), [v0.5.8-alpha](docs/RELEASE-v0.5.8-alpha.md), [v0.5.7-alpha](docs/RELEASE-v0.5.7-alpha.md), [v0.5.6-alpha](docs/RELEASE-v0.5.6-alpha.md), [v0.5.5-alpha](docs/RELEASE-v0.5.5-alpha.md), [v0.5.4-alpha](docs/RELEASE-v0.5.4-alpha.md), [v0.5.3-alpha](docs/RELEASE-v0.5.3-alpha.md), [v0.5.2-alpha](docs/RELEASE-v0.5.2-alpha.md), [v0.5.1-alpha](docs/RELEASE-v0.5.1-alpha.md), [v0.5.0-alpha](docs/RELEASE-v0.5.0-alpha.md), [v0.4.9-alpha](docs/RELEASE-v0.4.9-alpha.md), [v0.4.8-alpha](docs/RELEASE-v0.4.8-alpha.md), [v0.4.7-alpha](docs/RELEASE-v0.4.7-alpha.md), [v0.4.6-alpha](docs/RELEASE-v0.4.6-alpha.md), [v0.4.5-alpha](docs/RELEASE-v0.4.5-alpha.md), [v0.4.4-alpha](docs/RELEASE-v0.4.4-alpha.md), [v0.4.3-alpha](docs/RELEASE-v0.4.3-alpha.md), [v0.4.2-alpha](docs/RELEASE-v0.4.2-alpha.md).
