...
**Hardening: dual-signal spiral detection + periodic self-healing monitor**
- Field testing of v0.8.4 on busy boxes (Mission API / DataSync) showed normal high-volume `Bind request` traffic could push spiral markers off the `--tail 200` window the detector sampled. Boxes were actively spiraling but the migration logged "0/2 markers — leaving alone".
- New `_detect_authentik_ldap_spiral()` confirms a spiral via **either** signal: ≥2 unique markers in last **1000** outpost log lines (was 200), **or** ≥30 connections in `idle in transaction` from `application_name LIKE '%authentik%'` in Postgres. The Postgres signal is durable — survives container recreates and can't be drowned out by bind volume.
- New `_authentik_spiral_monitor()` daemon thread runs every 10 minutes; if a spiral is confirmed, runs the same idempotent `_apply_authentik_ldap_routing_repair`. Same gates, same Caddy probe, same auto-rollback. Single-instance PID-checked lock (only one gunicorn worker runs the monitor). Repair rate-limited to 1 per 6 hours; attempts persisted to `settings.authentik_spiral_last_repair`.
- Every early-return in the routing repair now logs **why** it skipped under the `routing repair: ...` prefix — silent skips can no longer hide bugs.
