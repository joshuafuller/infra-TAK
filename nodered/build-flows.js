...
const configFlows = [
  ...
  // expected type, and re-saves it via global.set() so disk converges to the right shape.
  {
    ...
    func: [
      ...
      "  ipaws_config:   'object'",
      "};",
      "var fixed = [], initialized = [];",
      "Object.keys(EXPECTED).forEach(function(k) {",
      "  var orig = global.get(k);",
      "  // Missing key — initialize to empty so engines and saves never see undefined",
      ...
      "  }",
      "  var n = _norm(orig);",
      "  // Type coerce — if expected array but got something else, replace with []",
      "  // (better to lose data than have engines silently crash on every poll)",
      "  if (EXPECTED[k] === 'array' && !Array.isArray(n)) n = [];",
      "  if (EXPECTED[k] === 'object' && (typeof n !== 'object' || Array.isArray(n) || n === null)) n = {};",
      "  // Only re-set if normalization actually changed something",
      "  if (n !== orig) {",
      "    global.set(k, n);",
      ...
      "if (!fixed.length && !initialized.length) node.warn('Context auto-heal: all keys clean');",
      "return null;"
    ].join('\n'),
    outputs: 1, timeout: '', noerr: 0,
    initialize: '', finalize: '', libs: [],
  ...
  },
  {
    ...
    func: [
      ...
      "  return v;",
      "}",
      "// Force result into expected shape — better to lose the value than to store a string",
      "// where engines expect an array (bug found 0.7.4-alpha: arcgis_configs ended up as a",
      "// JSON-stringified literal `\"[...]\"`, breaking every engine tab without UI feedback).",
      "var EXPECTED = {",
      "  arcgis_configs: 'array',",
      "  tc_configs:     'array',",
      "  pp_configs:     'array',",
      ...
      "};",
      "function coerce(k, v) {",
      "  if (EXPECTED[k] === 'array')  return Array.isArray(v) ? v : [];",
      "  if (EXPECTED[k] === 'object') return (v && typeof v === 'object' && !Array.isArray(v)) ? v : {};",
      "  return v;",
      "}",
      "var restored = [];",
      ...
      "  if (d[k] !== undefined) {",
      "    global.set(k, coerce(k, unwrapCtxVal(d[k])));",
      "    restored.push(k);",
      "  } else if (global.get(k) === undefined || global.get(k) === null) {",
      "    // Initialize empty so downstream engines/saves never see undefined",
      "    global.set(k, EXPECTED[k] === 'array' ? [] : {});",
