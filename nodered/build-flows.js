...
const FN_IPAWS_BUILD_KML = [
  ...
  "var nWithGeo = 0;",
  "",
  "// State/territory centroids [lon, lat] for zone-based alerts without polygon geometry",
  "var STATE_CENTROIDS = {",
  ...
  "  // NWS marine zone prefixes",
  "  'AN':[-70.0,40.5],'AM':[-78.0,32.0],",
  "  'GM':[-89.5,27.0],",
  "  'PZ':[-124.5,38.5],'PH':[-157.5,21.0],'PK':[-153.0,57.5],",
  "  'BZ':[-170.0,60.5],'AJ':[-135.0,58.0],",
  "  'LS':[-86.5,47.0],'LM':[-87.0,44.0],'LH':[-83.0,44.5],",
  "  'LE':[-81.0,42.0],'LO':[-77.0,43.5],'LC':[-73.2,44.5],",
  "  'SL':[-70.0,47.0]",
  "};",
  "",
  ...
  "  } else {",
  "    // Zone-based alert — fall back to state centroid derived from geocode.UGC",
  ...
  "    if (!stXY) return; // unknown state/territory, skip",
  ...
  "",
  "  // Polygon(s) — only for alerts with actual polygon geometry",
  ...
  "  '</Document></kml>'",
  "].join('\\n');",
  "node.warn('IPAWS KML: ' + features.length + ' total alerts → ' + nWithGeo + ' mapped (' + (features.length - nWithGeo) + ' skipped — unknown state)');",
  "return msg;"
].join('\n');
